...
module HerokuResqueAutoScale
  ...
  def after_enqueue_scale_up(*args)
    HerokuResqueAutoScale::Config.thresholds(@queue).reverse_each do |scale_info|
      # Run backwards so it gets set to the highest value first
      # Otherwise if there were 70 jobs, it would get set to 1, then 2, then 3, etc

      # If we have a job count greater than or equal to the job limit for this scale info
      if Scaler.job_count(queue) >= scale_info[:job_count]
  ...
  def scale_down
    ...
    Scaler.scale(@queue,  0) if Scaler.job_count(@queue).zero? && Scaler.working_job_count(@queue) == 1
  end
end